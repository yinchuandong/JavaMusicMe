...
//		if (str.startsWith("http://") && str.endsWith(".mp3")) {
	 ...
	 * @param fromPos 从第几秒开始
	 * @return 成功播放指定的文件返回true，否则返回false。
	 */
	public boolean start(boolean verbose, int fromPos) {
		Layer123 layer = null;
		int frames = 0;
		paused = false;
		...
		try {
			...
			//设置初始播放的偏移位置
			if (fromPos > 0) {
				int framecounts = (int)(fromPos / header.getFrameDuration());
				instream.seek(header.getFrameSize()*framecounts);
				header.setFrames(framecounts);
			}
